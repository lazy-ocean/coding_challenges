...
const tasks = require("../src/01-strings");
...
describe("01-strings-tasks", () => {
  ...
  test("getStringLength should return the length of string", () => {
  ...
  test("getStringFromTemplate should create a string from template using given parameters", () => {
  ...
  test("getFirstChar should return the first char from given string", () => {
    assert.equal(tasks.getFirstChar("John Doe"), "J");
    assert.equal(tasks.getFirstChar("cat"), "c");
  });

  test("extractNameFromTemplate should parse the name from given string", () => {
  ...
  test("removeLeadingAndTrailingWhitespaces should remove leading and trailing whitespaces from the string", () => {
  ...
  test("repeatString should repeat string specified number of times", () => {
    assert.equal(tasks.repeatString("A", 5), "AAAAA");
    assert.equal(tasks.repeatString("cat", 3), "catcatcat");
  });

  test("removeFirstOccurrences should remove all specified values from a string", () => {
  ...
  test("unbracketTag should remove first and last angle brackets from tag string", () => {
    assert.equal(tasks.unbracketTag("<div>"), "div");
    assert.equal(tasks.unbracketTag("<span>"), "span");
    assert.equal(tasks.unbracketTag("<a>"), "a");
  });

  test("convertToUpperCase should convert all chars from specified string into upper case", () => {
  ...
  test("extractEmails should extract emails from string list delimeted by semicolons", () => {
    assert.deepEqual(
  ...
  test("getRectangleString should return the string reprentation of rectangle with specified size", () => {
    assert.equal(
  ...
  test("encodeToRot13 should encode-decode string using ROT13 algorithm", () => {
  ...
  test("isString should return true if argument is a string", () => {
  ...
  test("getCardId should return the index of card in the initial deck", () => {
    [
      "A♣",
      "2♣",
      "3♣",
      ...
      assert.equal(tasks.getCardId(val), index, `Invalid id for card '${val}':`);
    });
  });
});
