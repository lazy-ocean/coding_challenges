...
const getAverage = (num1, num2) => {
  if (num1 + num2 > Number.MAX_VALUE) return Number.MAX_VALUE;
  return (num1 + num2) / 2;
};
...
const getAngleBetweenVectors = (x1, y1, x2, y2) => {
  ...
  const result = (dot / length(x1, y1)) * length(x2, y2);
 ...
 * '-525.5'     => -525.5
 */
const parseNumberFromString = (str) => +str;
...
const toNumber = (value, def) => {
  ...
  return typeof result === 'number' && !isNaN(result) ? result : def;
};
