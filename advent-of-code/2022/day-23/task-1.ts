...
export const findAvailableSpot = (
  map: string[],
  i: number,
  j: number,
  dir: Directions
): { iM: number; jM: number } => {
  const directionsOrder = directionsMapping[dir];
  // eslint-disable-next-line no-restricted-syntax
  for (const currDir of directionsOrder) {
    let potentialMove = null;
    if (currDir === Directions.N || currDir === Directions.S) {
      if (checkDirVert(map, i, j, currDir)) {
        potentialMove =
          currDir === Directions.N
            ? { iM: i - 1, jM: j }
            : { iM: i + 1, jM: j };
      }
    } else if (currDir === Directions.E || currDir === Directions.W) {
      if (checkDirHor(map, i, j, currDir))
        potentialMove =
          currDir === Directions.W
            ? { iM: i, jM: j - 1 }
            : { iM: i, jM: j + 1 };
    }
...
export const simulateDiffusion = (data: string[]): string[] => {
  let map = [...data];
...
const countEmptyTiles = (map: string[]): number => {
  let maxX = Number.MIN_SAFE_INTEGER;
  let minX = Number.MAX_SAFE_INTEGER;
  let maxY = Number.MIN_SAFE_INTEGER;
  let minY = Number.MAX_SAFE_INTEGER;

  map.forEach((elf: string) => {
  ...
  const squareArea = (maxX - minX + 1) * (maxY - minY + 1);

  return squareArea - map.length;
};

console.log(countEmptyTiles(simulateDiffusion(findElves(input))));
