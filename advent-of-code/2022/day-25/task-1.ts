...
const dataset: string = fs.readFileSync(
  "advent-of-code/2022/day-25/data.txt",
  "utf8",
  () => null
);
...
const numToSnafu = (num: number): string => {
  ...
  let curr = num;
  while (curr > 0) {
    let rem: string | number = ((curr + 2) % 5) - 2;
    curr = Math.floor((curr + 2) / 5);
    if (rem === -2) rem = "=";
    if (rem === -1) rem = "-";
...
const snafuToNum = (str: string[]): number =>
  eval(
    str
      .reverse()
      .map((item, i) => {
...
const findSum = (nums: string[][]): number => {
  let sum = 0;
  // eslint-disable-next-line no-return-assign
  nums.forEach((num) => (sum += snafuToNum(num)));
  return sum;
};

console.log(numToSnafu(findSum(data)));
