...
const isVisibleToLeft = (row: number[], itemI: number): boolean => {
  let isVisible = true;
...
const isVisibleToRight = (row: number[], itemI: number): boolean => {
  let isVisible = true;
...
const countVisibleTrees = (data: number[][]): number => {
  ...
  const outsideTrees = data[0].length * 2 + (data.length - 2) * 2;
  let isVisible = outsideTrees;
