...
const findDangerousAreas = (data: Coordinate[]) => {
  ...
  data.forEach(({ x1, x2, y1, y2 }) => {
    if (x1 === x2) {
      if (!visitedCoordinates[x1]) visitedCoordinates[x1] = {};
      for (let i = Math.min(y1, y2); i <= Math.max(y1, y2); i++) {
        if (!visitedCoordinates[x1][i]) {
          visitedCoordinates[x1][i] = 0;
        } else if (visitedCoordinates[x1][i] === 1) {
          dangerousAreas++;
        }
        visitedCoordinates[x1][i]++;
      }
    }
    if (y1 === y2) {
      for (let i = Math.min(x1, x2); i <= Math.max(x1, x2); i++) {
        if (!visitedCoordinates[i]) {
          visitedCoordinates[i] = { [y1]: 0 };
        } else if (!visitedCoordinates[i][y1]) {
          visitedCoordinates[i][y1] = 0;
        } else if (visitedCoordinates[i][y1] === 1) {
          dangerousAreas++;
        }
        visitedCoordinates[i][y1]++;
      }
    }
  });
