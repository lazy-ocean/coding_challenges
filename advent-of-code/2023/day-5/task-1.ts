...
const parseData = (): {
  seeds: number[];
  info: { dest: number; source: number; maxSource: number }[][];
} => {
  const [seedsData, ...rest] = input;
  const seeds = seedsData[0].split(": ")[1].split(" ").map(Number);
...
const findWinningNumsSum = (): number => {
  ...
  seeds.forEach((seed) => {
    ...
    info.forEach((rule) => {
      for (let i = 0; i < rule.length; i++) {
        const { dest, source, maxSource } = rule[i];
        if (currSource >= source && currSource <= maxSource) {
          currSource = currSource - source + dest;
          break;
        }
      }
    });
