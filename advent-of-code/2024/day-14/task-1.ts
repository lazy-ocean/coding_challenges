...
const findPositions = () => {
  ...
  input.forEach((line) => {
    positions.push(findPos(line));
  });

  const middlex = Math.ceil(maxx / 2) - 1;
  const middley = Math.ceil(maxy / 2) - 1;
  positions.forEach((pos) => {
    const [x, y] = pos;
    if (x < middlex && y < middley) quadrants[1]++;
    if (x < middlex && y > middley) quadrants[2]++;
    if (x > middlex && y < middley) quadrants[3]++;
    if (x > middlex && y > middley) quadrants[4]++;
  });

  return Object.values(quadrants).reduce((a, b) => a * b);
};
