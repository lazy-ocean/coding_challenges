...
const findNextFrequences = ({
  ...
  const next = [x2 + distX, y2 + distY];
  const nextup = [x1 - distX, y1 - distY];

  return [next, nextup];
};

const checkIfWithinBounds = (dot: number[]): boolean => {
  const [x, y] = dot;
  ...
  Object.keys(locationsMap).forEach((antenna) => {
    ...
    for (let i = 0; i < alllocations.length; i++) {
      ...
      for (let j = i + 1; j < alllocations.length; j++) {
        ...
        const [up, down] = findNextFrequences({ dot1, dot2 });

        if (checkIfWithinBounds(up)) {
          res.add(`${up[0]};${up[1]}`);
        }
        if (checkIfWithinBounds(down)) {
          res.add(`${down[0]};${down[1]}`);
        }
      }
    }
  });
