...
const dataset: string = fs.readFileSync(
  "advent-of-code/2024/day-22/data.txt",
  "utf8",
  () => null
);
...
const mix = (value: bigint, secret: bigint): bigint => {
  const overlap = value & secret;
  const item = value ^ secret;
  return overlap === 0n ? value + secret : BigInt(item);
};

const MAGIC_NUM = 16777216;

const prune = (secret: bigint): bigint => secret % BigInt(MAGIC_NUM);

const steps = (secret: bigint): bigint => {
  const firstStepTemp = mix(secret * BigInt(64), secret);
  const firstStep =
    firstStepTemp < BigInt(MAGIC_NUM) ? firstStepTemp : prune(firstStepTemp);
  const secondStepTemp = mix(firstStep / BigInt(32), firstStep);
  const secondStep =
    secondStepTemp < BigInt(MAGIC_NUM) ? secondStepTemp : prune(secondStepTemp);

  const thirdStepTemp = mix(secondStep * BigInt(2048), secondStep);
  const thirdStep =
    thirdStepTemp < BigInt(MAGIC_NUM) ? thirdStepTemp : prune(thirdStepTemp);
  return thirdStep;
};

const try2000times = (n: bigint): bigint => {
  let step = 0;
  let curr = n;
  while (step < 2000) {
    curr = steps(BigInt(curr));
    step++;
  }
  return curr;
};

const tryForEachNumber = () => {
  let sum = 0n;
  input.forEach((num) => {
    sum += try2000times(BigInt(num));
  });
  return sum;
};

console.log(tryForEachNumber());
