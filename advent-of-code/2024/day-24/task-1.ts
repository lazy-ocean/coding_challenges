...
const dataset: string = fs.readFileSync(
  "advent-of-code/2024/day-24/data.txt",
  "utf8",
  () => null
);

const [wiresData, gatesData] = dataset.split("\n\n");
const zWires = [];

const wires = wiresData.split("\n").reduce((acc, item) => {
...
const operationOr = (a: number, b: number): number => (a || b ? 1 : 0);
const operationXor = (a: number, b: number): number => (a !== b ? 1 : 0);
const operationAnd = (a: number, b: number): number => (a && b ? 1 : 0);

const operationsMap = {
  [Operation.and]: (a: number, b: number) => operationAnd(a, b),
  [Operation.xor]: (a: number, b: number) => operationXor(a, b),
  [Operation.or]: (a: number, b: number) => operationOr(a, b),
};

const gates = gatesData.split("\n");

const goThruInstructions = (): number => {
  const gatesQueue = [...gates];

  while (gatesQueue.length) {
    const item = gatesQueue.shift();
    const [instr, res] = item.split(" -> ");
    const [a, op, b] = instr.split(" ");

    if (wires[a] === undefined || wires[b] === undefined) {
      gatesQueue.push(item);
    } else {
      const result = operationsMap[op](wires[a], wires[b]);
      wires[res] = result;
      if (res.startsWith("z")) zWires.push(res);
    }
  }

  zWires.sort((a, b) => Number(b.substring(1)) - Number(a.substring(1)));
  const res = parseInt(zWires.map((item) => wires[item]).join(""), 2);

  return res;
};

console.log(goThruInstructions());
