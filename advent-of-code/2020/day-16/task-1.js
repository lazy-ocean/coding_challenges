...
const countInvalidTickets = (nearbyTickets) => {
  let invalid = [];
  nearbyTickets.forEach((ticket) => {
    ticket.forEach((number) => {
      if (!checkAgainstRules(number)) invalid.push(number);
    });
  });
  return _.sum(invalid);
};

const checkAgainstRules = (number) => {
...
console.log(countInvalidTickets(nearbyTickets));
