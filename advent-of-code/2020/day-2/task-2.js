...
function func(arr) {
  let filter = arr.filter((str) => {
    let interval = str.split(" ");
    let nums = interval[0].split("-");
    let letter = interval[1].slice(0, 1);
    let password = interval[2];
    return (
      (password.split("")[nums[0] - 1] === letter &&
        password.split("")[nums[1] - 1] !== letter) ||
      (password.split("")[nums[0] - 1] !== letter &&
        password.split("")[nums[1] - 1] === letter)
    );
  });
