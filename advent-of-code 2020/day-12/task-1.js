...
const findDirections = (input) => {
  ...
  input.forEach((direction) => {
    let face = currPosition.faced;
    if (keys.includes(direction.dir)) {
      currPosition[direction.dir] += direction.num;
    } else if (direction.dir === "F") {
      currPosition[face] += direction.num;
    } else {
      let turns = direction.num / 90;
      let newList = list[direction.dir][face];
      currPosition.faced = newList[turns];
    }
  });
  let manhDist =
